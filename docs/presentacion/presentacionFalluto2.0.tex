...
\item $L: S → P(AP)$ función de etiquetado o interpretación.
\end{itemize}
\end{frame}
%_____________________________________________________________
...
Se compila como:
\begin{framed}
$\texttt{act\#var'}~=~trans~\wedge~!stop~\wedge~pre~\wedge~pos1~\wedge~pos2~\wedge...\wedge~unchanged$
\end{framed}
Donde:
...
\begin{framed}
$\texttt{act\#var'} = syncname~\wedge~!stop~\wedge~pre1~\wedge~pre2~\wedge~...$
...
Compila a:
\begin{framed}
$\texttt{act\#var'} = fault~\wedge~!fault\_active~\wedge~pre~\wedge~fault\_active'$
...
\begin{framed}
$\texttt{act\#var' = dk\#trans}~\wedge~neg\_pre~\wedge~unchanged$
\end{framed}
...
Compilación:
\begin{framed}
$\texttt{FAIRNESS}~(\bigvee_{t \in T_{N_i}} \texttt{act\#var} = t)~\vee~instanceDK$
\end{framed}
