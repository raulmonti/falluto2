""" El ejemplo de parseo 5 = ( x | y ) no se parsea bien con GRAMMAR cuando 
uso packrat en pypeg :S
"""
...
def COMPARISON():   return [ (MATHEXP, re.compile(r"\>|\<|\>\=|\<\="), MATHEXP)
                           , (COMPVAL, re.compile(r"\=|\!\="), COMPVAL)
                           ]
def COMPVAL():      return [ (re.compile(r"\("), COMPVAL, re.compile(r"\)"))
                           , (re.compile(r"\!"), COMPVAL)
                           , IDENT
                           , COMPLEXID
                           , EVENT
                           , NEXTREF
...
def BOOLEXP():      return BOOLFORM, 0, (re.compile(r"\-\>|\<\-\>"), BOOLEXP) 
def BOOLFORM():     return BOOLVAL, 0, (re.compile(r"\&|\|"), BOOLFORM)
def BOOLVAL():      return [ COMPARISON
                           , (re.compile(r"\("), BOOLEXP, re.compile(r"\)"))
...
if __name__ == "__main__":
    ...
    def TEST(): return [BOOLEXP]

    _file = fileinput.input()
    _ast = parse(TEST, _file, True, packrat = False)
