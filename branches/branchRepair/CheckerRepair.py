...
import ExceptionsRepair
from TypesRepair import *
from UtilsRepair import _cl, ss
...
def Check(model):
    ...
    """
    if not (isinstance(model, ParserRepair.Model)):
...
class Checker(object):
    ...
    def __init__(self):
        ...
        self.globalinst      = ParserRepair.Instance()
    ...
    def check(self, model):
        assert isinstance(model, ParserRepair.Model)
        self.clear()
        self.mdl = model
    ...
    def getPROPType(self, inst, ast):
        ...
        assert isinstance(inst, ParserRepair.Instance)
        assert isinstance(ast, pyPEG.Symbol)
...
if __name__ == "__main__":
    ...
    try:
        ...
        _mdl = ParserRepair.parse(_file)
        DebugRepair.debug("debugGREEN", "Checking ...")
        Check(_mdl)
        DebugRepair.debug("debugGREEN", "it's OK!")
    except Error, _e:
